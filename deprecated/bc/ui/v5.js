...
(function () {
  ...
  const LOWS_LOOKBACKS = [10, 50, 100];
  const LOOKBACKS = [100, 500, 1000];
  ...
  function renderTable() {
    ...
    TARGET_MANAGER.getTargets().forEach((entry) => {
      ...
      const compressionIndex = entry.getCompressionIndex(1000);
      let compressionIndexColor =
      ...
      const rollOverTil10X = entry.getRollOverTil10X();
      ...
      <tr>
        ...
        <td style='color: ${streakTextColor}; background-color: ${streakColor}'><span id="roll-over-${sanitized}">${rollOverTil10X}</span></td>
...
<td style='background-color: ${compressionIndexColor}'><span id="compression-index-${sanitized}">${compressionIndex.toFixed(
        2
      )}</span></td>
  ...
  function createHitDeltaCells(deltas) {
    const data = [];

    for (let i = 0; i < deltas.length; i++) {
      data.push(
        `<span style="background-color: ${deltas[i].bgColor}">${deltas[i].delta}</span>`
      );
    }
    return data.join(" | ");
  }
  ...
  function createWinRateCells(entry) {
    ...
    const uiData = entry.getWinRatesUIData();
    const rowData = uiData
      .map((data) => {
  ...
  function generateTargetManager() {
    class Stats {
      ...
      getCompressionIndex = (lookback) => {
        lookback = !lookback ? this.results.length : lookback;
      ...
      getWinRatePercentsOfTeo() {
        return this.lookbacks.map((lookback) =>
          this.calculateWinRatePercentOfTeo(this.getWinRate(lookback))
        );
      }
      ...
      getHitDelta(lookback) {
        const lb = this.globalRolls < lookback ? this.globalHits : lookback;
      ...
      getHitDeltas() {
        const lookbacks = [
          Math.ceil(this.payout * 5),
          Math.ceil(this.payout * 10),
          Math.ceil(this.payout * 50)
        ];

        return lookbacks.map((lookback) => {
          const delta = this.getHitDelta(lookback);
          const bgColor = this.getStreakColor(delta);
    ...
    class Target {
      constructor(payout, decay = 0.95) {
        ...
        this.stats = new Stats(
          payout,
          this.payout <= 2 ? LOWS_LOOKBACKS : LOOKBACKS
        );
      ...
      getHitDelta(lookback) {
        return this.stats.getHitDelta(lookback);
      }
      ...
      getWinRatePercentsOfTeo = () => this.stats.getWinRatePercentsOfTeo();
      getRollOverTil10X = () => {
      ...
      getWinRatesUIData() {
        const rates = this.stats.getWinRates();
        const color = this.getWinRateColor(rates);
        const uiData = [];
        for (let i = 0; i < rates.length; i++) {
          const data = {};

          const pot = this.calculateWinRatePercentOfTeo(rates[i]);

          data.winRate = `${(rates[i] * 100).toFixed(2) } | ${(pot * 100).toFixed(2)}`;
          data.pot = pot;
          data.bgColor = color;
          uiData.push(data);
        }
      ...
      getWinRateColor(rates) {
        const pot = this.calculateWinRatePercentOfTeo(rates[0]);
        const pot2 = this.calculateWinRatePercentOfTeo(rates[1]);
        const pot3 = this.calculateWinRatePercentOfTeo(rates[2]);

        let bgColor = "transparent";

        if ((pot > 1 && pot2 > 1 && pot3 > 1) || (pot < 0.1 && pot2 < 0.5 && pot3 < 0.9) || (pot < 0.1 || pot2 < 0.5 || pot3 < 0.7)) {
        ...
        bgColor = `rgba(${baseColor[0]}, ${baseColor[1]}, ${baseColor[2]}, ${alpha})`;
                }
  ...
  function generateWatchPayouts() {
    const a1 = [1.1, 1.2, 1.3, 1.5, 1.5, 1.6, 1.7, 1.8, 1.9, 2];
  ...
  function injectUI() {
    ...
    const uiHTML = `<div id="ui-v4-stats-panel" style="
   position: fixed;
   top: 300px;
   left: 500px;
   z-index: 9999;
   ...
   cursor: grab;
   ">
   <div id="ui-v4-stats-panel-header" style="
      background: #333;
      padding: 10px;
      font-weight: bold;
      text-align: center;
