...
(function () {
  ...
  function renderTable() {
    ...
    targets.forEach((entry) => {
      ...
      const wrp = entry.getWindowWinRatePercentsOfTeo();

      const teo = entry.getTeo() * 100;
      const ri = entry.rarityIndex.getStats();
      const rates = entry.getWindowWinRates();
      const wr1 = rates[0];
      const wr1Color = getWinRateColor(wrp[0] - 100);
      const wr2 = rates[1];
      const wr2Color = getWinRateColor(wrp[1] - 100);
      const wr3 = rates[2];
      const wr3Color = getWinRateColor(wrp[2] - 100);;
      ...
      <td><span>${teo.toFixed(2)}</span> | <span style="background-color: ${wr1Color}">${wr1.toFixed(2)}</span> | <span style="background-color: ${wr2Color}">${wr2.toFixed(2)}</span> | <span style="background-color: ${wr3Color}">${wr3.toFixed(2)}</span></td>
      </tr>
    `;
      $tbody.append(row);
    });
  ...
  function generateTargetManager() {
    class Stats {
      constructor(payout, lookbacks = [20, 100, 1000]) {
      ...
      getWindowWinRatePercentsOfTeo() {
        return this.lookbacks.map((lookback) => ((this.getWindowWinRate(lookback) / 100) / this.getTeo()) * 100);
      }
