...
`;
    ...
    function getNextRiskTarget(result) {
        if (currentRiskOnTarget === null) {
            currentRiskOnTarget = getLosingTopTargetInLosingStreak();
        }
        ...
        if (!currentRiskOnTarget.isRiskOn()) {
            ...
            const wager = getTieredWager(currentRiskOnTarget)
            currentRiskOnTarget.goRiskOn();
            ...
            const payout = currentRiskOnTarget.getPayout();
            const wager = getTieredWager(currentRiskOnTarget)
            console.log(`Updating risk on with wager ${wager}`)
            setWager(wager);
            return;
        }
    }
    ...
    function evalRiskState(result) {
        ...
        if (result >= currentRiskOnTarget.getPayout()) {
            if (riskOnReTryCount === 0)
            {
                currentRiskOnTarget.goRiskOff();
                currentRiskOnTarget = null;
                riskOnLosingStreak = 0;
                setPayout(1.01);
                setWager(0);
                maxRatio = 0;
            }
        ...
        else
        {
            ...
            if ((riskOnLosingStreak - 1) % Math.floor(currentRiskOnTarget.getPayout()) === 0)
            {
                riskOnReTryCount++;
            }
        }
    ...
    function getLosingTopTargetInLosingStreak() {
        ...
        const results = TARGETS.filter(target => Math.abs(target.getLosingStreak()) > (target.getPayout() * lsm))
    ...
    function startBetting() {
        const payout = getPayout(); // Cache it
        const maxRolls = getMaxRolls(); // Cache it
        const stopOnWin = getStopOnWin();
        if (!getStopOnWin()) setWager(0)
        SESSION.start();
        doBet();
    }
    ...
    function generateTargets() {
        ...
        class Target {
            ...
            getLosingStreakGreaterThan(n) {
                return this.getLosingStreak() > this.payout * n
            }

            getLosingStreak() {
                let streak = 0

                // Iterate from the last roll backward
                for (let i = this.rolls.length - 1; i >= 0; i--) {
                    if (this.rolls[i] < this.payout) {
                        streak++ // Increase streak count if roll is a loss
                    } else {
                        break // Stop counting if a winning roll is found
                    }
                }
            ...
            getRatio() {
                return this.getLosingStreak() / this.payout;
            }
        }
        return payouts.map((payout) => new Target(payout, ROLLS))
    }
