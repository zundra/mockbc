/* Start Script */
// ==UserScript==
// @name         bc.game progressive bet on mod v5
...
setWager(currentWager, isHit) {
  ...
  this.wager = this.baseWager + (this.baseWager - (this.baseWager * ((winRatePercentOfTeo / 0.01) / 100)))
}
...
class Target {
  ...
  tryRiskOn(currentRoll) {
    ...
    if (shortMADirection === 1 && long < 100 && short > long) {
  ...
  tryRiskOff(currentRoll) {
    ...
    if (shortMADirection === -1 || long > 150 || short < long) {
 ...
 // Push new TEO diff value
  ...
  getRiskOnPerformance() {
    ...
    ">
        ...
        <div>Short Percent of Teo MA: <span id="short-win-rate-percent-of-teo-ma"></span></div>
        <div>Short Percent of Teo Direction: <span id="short-win-rate-percent-of-teo-direction"></span></div>
...
`;
   ...
   let riskOnTarget = null;
    ...
    function updateUI() {
        ...
        const maDirection = riskOnTarget.getMovingAverageDirection(5);
        const maDirectionLabel = maDirection === -1 ? "Down" : (maDirection === 1 ? "UP" : "Neutral")
        const ma = riskOnTarget.getTeoDiffMovingAverage(5);
      ...
      $("#short-win-rate-percent-of-teo-ma").html(`${ma.toFixed(2)}`)
      $("#short-win-rate-percent-of-teo-direction").html(`${maDirectionLabel}`)
