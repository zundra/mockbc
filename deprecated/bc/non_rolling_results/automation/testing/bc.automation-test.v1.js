...
(function () {
  ...
  function initAutomationManager() {
    class AutomationManager {
      ...
      checkRiskOn(result) {
        console.log("GO RISK ONFF?");
        if (result >= this.riskOnTarget.getPayout()) {
          console.log("SHOULD GO RISK OFF");
          this.goRiskOff();
        }
      }

      goRiskOn() {
        const wager = this.getScaledWager(this.riskOnTarget.getPayout());
        console.log(
          `Going risk on with target ${this.riskOnTarget.getPayout()} and wager ${wager}`
        );
        this.riskOnTarget.goRiskOn(wager);
      ...
      goRiskOff() {
        console.log(
          `Going risk off with target ${this.riskOnTarget.getPayout()} and wager ${wager}`
        );
