...
(function () {
  ...
  */
  ...
  function evalRiskOn() {
    ...
    if (tm.shouldExitPosition(riskOnTarget)) {
      clearRiskOn();
      return;
    }

    const nextTarget = tm.getNextRiskTarget();

    if (nextTarget) {
      const wager = riskOnTarget.getWager();
      setRiskOn(nextTarget, wager);
    }
  }
  ...
  function evalRiskOff() {
    ...
    console.log("Risk On Target", riskOnTarget.getPayout(), "going risk on");
  ...
  function setRiskOn(target, wager) {
    clearRiskOn();

    if (!target || !wager) return;
    riskOnTarget = target;
    riskOnTarget.goRiskOn(wager);
    setWager(riskOnTarget.getWager());
    setPayout(riskOnTarget.getPayout());
  }
  ...
  function updateUI() {
    ...
    const s = tm.getRatioSlope();

    $("#total-ratio")
      .html(`${r.toFixed(3)} | ${lastResult}`)
      .css({ backgroundColor: `${r < 0 ? "red" : "transparent"}` });

    $("#ratio-slope")
      .html(`${s.toFixed(3)}`)
      .css({ backgroundColor: `${s < 0 ? "red" : "transparent"}` });
  ...
  function initTargetManager(scenario) {
    ...
    class Target {
      ...
      evalRiskOn() {
        ...
        // } else {
        //   if (this.losingStreak % Math.floor(this.payout / 2) === 0) {
        //     this.wager = Math.min(0.25, this.wager *= 2);
        //   }
        // }
      }
  ...
  `;
      ...
      setRecoveryFlips() {
        ...
        this.flipsDelta = this.currentFlips.filter(x => !this.previousFlips.includes(x));
      }
...
function deepEqual(obj1, obj2) {
  ...
  function injectStatsPanel() {
   ...
   <div class="control-group">
      ...
      <input type="number" id="base-wager" value="0.002"  />
   </div>
   <div class="control-group">
   ...
   <div class="control-group">
      ...
      <input type="number" id="hard-stop-max-loss" value="15"  />
   </div>
   <div class="control-group">
