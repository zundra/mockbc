...
class Automation {
  ...
  evaluateRiskOn(result) {
    ...
    if (this.profit > 0) {
      ...
      console.log("Going risk off for target", this.riskOnTarget.getPayout());
      this.goRiskOff();
    } else {
...
const targets = populateTargets();
  ...
  class Target {
    ...
    isCompressed = (threshold = 1) =>
      this.getLosingRatio() >= threshold && this.getWinRatePercentOfTeo() < 100;
