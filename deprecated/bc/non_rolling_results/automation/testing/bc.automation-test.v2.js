...
(function () {
  ...
  const scenarios = {
    baseline: (target) => ({
      riskOnCheck: () => target.stats.getWinRateSensitiveLosingStreak() > 0,
  ...
  */
  ...
  function evalRiskOn(result) {
    ...
    if (result >= riskOnTarget.getPayout()) {
          console.log("HIT Risk On Target", riskOnTarget.getPayout(), "going risk off")
    ...
    riskOnTarget.goRiskOff();
    nextTarget.goRiskOn(getBaseWager());
...
class BankRoll {
    ...
    class Stats {
      ...
      updateStreak = (result) => {
        ...
        if (this.losingStreak === 1) {
          this.winRateWasLowWhenStreakStarted = this.getWinRatePercentOfTeo(100) < 80
      ...
      evalRiskOn() {
        if (this.progressiveBet()) {
          ...
          if (this.losingStreak === 0) {
            ...
            if (this.losingStreak % Math.floor(this.payout / divisor) === 0) {
                this.riskOnCycle++;
              this.wager = Math.min(0.50, this.wager + this.wager);
            }
          }
        }
      }
  ...
  `;
      ...
      riskOnReady() {
        return this.targets.find((t) => t.riskOnReady()) !== undefined
      }
