/* Start Script */
// ==UserScript==
// @name         bc.game reverse martingale
...
;(function () {
    ...
    const MAX_WAGER = 0.003;
    const MIN_WAGER = 0.0001;

    const RISK_ON_TARGET = initTarget()
    ...
    function evalResult(result) {
        if (SESSION.globalRollCount % 50 === 0) {
            console.log(
                `Roll Count: ${SESSION.globalRollCount}, P/L: ${RISK_ON_TARGET.getPL()}`,
            )
        }
    ...
    function updateUI() {
        $("#automation-bankRoll").text(RISK_ON_TARGET.getBankRollBalance())
        $("#automation-profitLoss").text(RISK_ON_TARGET.getPL())
    ...
    function setNextBet() {
        ...
        setPayout(RISK_ON_TARGET.getPayout())
        setWager(RISK_ON_TARGET.getWager())
    }
    ...
    function initTarget() {
        ...
        class RiskOnTarget {
            constructor(bankRollStartBalance) {
            ...
            setWager() {
                ...
                if (this.getLosingStreakAbs() >= this.getPayout()) {
                    this.wager = Math.min(MAX_WAGER, this.wager * 2)
                } else {
                    this.wager = Math.max(MIN_WAGER, this.wager / 2)
                }
            }
            getStreak() {
                return this.streak;
