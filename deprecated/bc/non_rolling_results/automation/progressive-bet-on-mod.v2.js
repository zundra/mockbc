/* Start Script */
// ==UserScript==
// @name         bc.game progressive bet on mod v2
...
class Target {
  ...
  tryRiskOn(currentRoll) {
    ...
    const shortMADirection = this.getMovingAverageDirection(5);
  ...
  tryRiskOff(currentRoll) {
    if (this.riskOnRollCount < (this.getPayout() * 2)) return;
    ...
    // Track if we ever reached above-TEO performance
    ...
    // // 🧹 Decay after above-TEO recovery
    // if (this.wasAboveTeo && short < 100) {
    //   this.resetRiskOnState(false);
    ...
    // // 🟡 Early exit after deep draw if profitable
    // if (
    //   this.deepDrawDetected &&
    ...
    // // 🔥 Exit grind mode if we’ve recovered to profit
 ...
 // Push new TEO diff value
  ...
  getMovingAverageDirection(lookback = 5) {
    ...
    ">
        ...
        <div>Cycle PL: <span id="cycle-pl"></span></div>
           ...
           <div class="control-group">
            ...
            <input type="number" id="base-wager" value="0.0001" style="color: black"/>
            </div>


           <div class="control-group">
            <label>Risk On Target</label>
            <input type="number" id="risk-on-target" value="2" style="color: black"/>
            </div>
...
//        checkHalts(result);
    ...
    function updateUI() {
        ...
        if (short < getShortPercentOfTeoThreshold()) {
          shortColor = "red";
        } else if (short > getShortPercentOfTeoThreshold()) {
          shortColor = "green";
        }
  ...
  function initWindowEvents() {
    ...
    function getCurrentRollData() {
     ...
     function getProgressiveWager() {
        let scalingFactor = Math.pow(2, Math.abs(losingStreak) / getPayout());
        return baseWager + parseFloat((baseWager * scalingFactor).toFixed(8));
      }
    ...
    function startBetting() {
        rollCount = 0;
        riskOnTarget = new Target(2, getBaseWager(), getShortPercentOfTeoThreshold(), getLongPercentOfTeoThreshold())
        baseWager = wager = 0.0001;
        currentTarget = 0;
        highHit.payout = 0;
        highHit.round = 0;
        teo = getTeo();
