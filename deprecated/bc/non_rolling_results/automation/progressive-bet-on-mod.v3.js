/* Start Script */
// ==UserScript==
// @name         bc.game progressive bet on mod v3
...
class RiskManager {
  ...
  addResult(currentWager, result, winRatePercentOfTeo) {
    this.rollCount++;
    if (result >= this.payout) {
      this.losingStreak = 0;
      this.winningStreak++;
      this.increment();
      this.setWager(currentWager, true, winRatePercentOfTeo);
    } else {
      this.losingStreak++;
      this.winningStreak = 0;
      this.decrement();
      this.setWager(currentWager, false, winRatePercentOfTeo);
    }
  }
...
setWager(currentWager, isHit, winRatePercentOfTeo) {
  ...
  // First roll after Risk On → start with base wager
  ...
  // Flat wager protection if WR% is high and already in profit
  if (winRatePercentOfTeo >= 105 && this.target.getRiskOnPL() > 0) {
    this.wager = this.baseWager;
    return;
  }
  ...
  // === High Payout Strategy (≥2x) ===
  const entryTeo = this.target?.riskOnStartPercentOfTeo || 100;
  const performance = winRatePercentOfTeo / entryTeo;

  // Base scale before adjustment (still relative to TEO entry snapshot)
  let scale = Math.max(this.baseWager / 2, Math.min(2, 2 - (performance / 0.8)));

  // === Scaling Ramp (based on # of rolls in Risk On) ===
  const rampRolls = this.target?.riskOnRollCount || 0;
  const rampMultiplier = Math.min(1, rampRolls / 10); // ramps over 10 rolls
  scale = 1 + (scale - 1) * rampMultiplier;
...
class Target {
  constructor(payout, baseWager, size = 100) {
  ...
  push(currentWager, result) {
    ...
    if (this.isRiskOn()) {
        this.riskOnRollCount = (this.riskOnRollCount || 0) + 1;

      this.setRiskOnPL(currentWager, result);
  ...
  setRiskOnPL(currentWager, result) {
    if (result >= this.payout) {
  ...
  tryRiskOn(currentRoll) {
    if (currentRoll < this.riskOnCooldownUntilRoll) return;
    ...
    this.riskOnStartPercentOfTeo = short;
    this.riskOnStartLongPercentOfTeo = long;
    this.wasAboveTeo = false;
    this.deepDrawDetected = false;
    this.deepDrawExitUsed = false;
    this.grindRecoveryMode = false;
...
tryRiskOff(currentRoll) {
  const short = this.getWinRatePercentOfTeo(100);
  const long = this.getWinRatePercentOfTeo(1000);
  const shortStart = this.riskOnStartPercentOfTeo;
  const longStart = this.riskOnStartLongPercentOfTeo;

  if (shortStart == null || longStart == null) return;

  // 🟢 Track if we've moved above 100% at any point
  if (!this.wasAboveTeo && short >= 100) {
    this.wasAboveTeo = true;
  }

  // 🧹 Exit once we've dropped back below 100% after recovery
  if (this.wasAboveTeo && short < 100) {
    this.resetRiskOnState(false);
    this.riskOnCooldownUntilRoll = currentRoll + 50;
    return;
  }

  // 🟡 Early exit from deep drawdown, but only if profitable
  if (
    this.deepDrawDetected &&
    !this.deepDrawExitUsed &&
    (short - shortStart) >= 10 &&
    this.getRiskOnPL() > 0
  ) {
    this.deepDrawExitUsed = true;
    this.resetRiskOnState(false);
    this.riskOnCooldownUntilRoll = currentRoll + 50;
    return;
  }

  // 🔥 Exit grind mode if we finally hit profit
  if (this.grindRecoveryMode && this.getRiskOnPL() > 0) {
    this.resetRiskOnState(false);
    this.riskOnCooldownUntilRoll = currentRoll + 50;
    return;
  }
  ...
  getWinRatePercentOfTeo = (lookback = 100) => {
    return (this.stats.getRate(lookback) / this.getTeo()) * 100;
  };
  ...
  getRiskOnPerformance() {
    ...
    ">
        ...
        <div>Percent of Teo: <span id="win-rate-percent-of-teo"></span></div>
...
`;
   ...
   let riskOnTarget = null;
    ...
    function updateUI() {
      ...
      $("#win-rate-percent-of-teo").html(`${riskOnTarget.getWinRatePercentOfTeo(100)}`);
    }
  ...
  function initWindowEvents() {
    ...
    function startBetting() {
        rollCount = 0;
        riskOnTarget = new Target(getRiskOnTarget(), getBaseWager())
        currentTarget = 0;
        highHit.payout = 0;
        highHit.round = 0;
        teo = getTeo();
