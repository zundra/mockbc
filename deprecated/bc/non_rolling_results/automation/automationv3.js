...
(function () {
  ...
  const scenarios = {
    negative: (target) => ({
      riskOnCheck: () => true,
      riskOffCheck: () => target.losingStreak >= target.payout
    }),
    baseline: (target) => ({
      riskOnCheck: () => Math.abs(target.getLosingStreak()) > target.payout * 4,
      riskOffCheck: () =>
        target.losingStreak >= target.payout * 2 ||
        target.getPL() >= target.getWager()
    }),

    winrate: (target) => ({
      riskOnCheck: () =>
        target.getWinRatePercentOfTeo() < 70 &&
        Math.abs(target.getLosingStreak()) > target.payout,
      riskOffCheck: () =>
