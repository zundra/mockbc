...
;(function () {
  ...
  function setRiskTarget(result) {
    const riskOnTarget = ROLL_HANDLER.getRiskOnTarget(getBaseWager())
  ...
  function evalRiskState() {
    if (!ROLL_HANDLER.getRiskOnRevoked()) return
      setMessage(ROLL_HANDLER.getRiskOnRevokedMessage());
      setNextBet(1.01, 0)
...
class RiskOnTarget {
  ...
  invoke(payout, wager, bankRoll) {
    ...
    this.wager = this.baseWager = wager;
  ...
  evalRiskState(result) {
    if (this.getPL() > 10) {
      this.revoke(`profit target hit for payout ${this.getPL()}`);
    }
  }
 ...
 //   if (this.isNotInvoked()) throw "Must invoke target before calling getWager";
  ...
  getTieredWager() {
    ...
    return this.getScaledWager() * this.maxRatio
  }

  getScaledWager(scalingFactor = 1.5) {
    return this.getAdjustedBaseWager() * Math.pow(1.92 / this.payout, scalingFactor)
  }

  getAdjustedBaseWager() {
        let drawdownPercent = (this.bankRoll.start - this.bankRoll.current) / this.bankRoll.start;
        console.log("DRAW DOWN PERCENT", drawdownPercent);

        return this.getBaseWager() * (1 + drawdownPercent);
    }
  ...
  setWager() {
  if (this.isNotInvoked()) throw "Must invoke target before calling setWager";
    
    if (this.wagerIncreaseDivisor === 1) {
      this.wager = this.getTieredWager();
      return;
    }

    if (this.rollCount % Math.floor(this.getPayout() / this.wagerIncreaseDivisor) === 0) {
      const w = this.wager;
      this.wager += this.wager;
      console.log("Increaseing wager from", w, "to", this.wager)
    }
  }
  ...
  isInvoked() {
    ...
    class RollHandler {
      ...
      getRiskOnTarget(wager) {
        if (this.riskOnTarget.isInvoked()) return this.riskOnTarget;

        const nextTarget = this.getNextRiskTarget();
      ...
      getNextRiskTarget() {
        return this.getTargetsWithStrengthBelowThreshold(0)
