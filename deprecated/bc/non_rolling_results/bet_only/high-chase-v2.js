/* Start Script */
// ==UserScript==
// @name         bc.game high chase with recovery v2
...
(function () {
  ...
  let controlPanel = `<div id="control-panel" style="
   ...
   <div id="stats" class="stats-box" style="
      ...
      <div>Sleep Count: <span id="sleep-count"></span></div>
  ...
  function evalResult(result) {
    ...
    if (rollModes.isSleepMode()) {
      evalSleepMode();
    } else if (rollModes.isHighMode()) {
    evalHighMode(result);
  ...
  function evalRiskOn(result) {
    ...
    riskOnTarget = tm.getNextHighestLowRatioTarget(riskOnTarget);

    if (riskOnTarget) {
      setPayout(riskOnTarget.getPayout());
      if (wager === 0) {
        wager = getBaseWager();
      } else if (losingStreak % Math.ceil(riskOnTarget.getPayout() / 2) === 0) {
        wager *= 2;
      }
  ...
  function checkHalts(result) {
    ...
    if (result >= 1000) {
      sleepRolls = Math.floor(Math.log2(result)) * BASE_SLEEP;

      if (!rollModes.isRecoveryMode()) {
        rollModes.setSleepMode();        
      }

      
      $("#message").text(`Hit hit detected, sleeping for ${sleepRolls} rolls`);
      return;
    }
  }
...
function getAvgRatioAtPop() {
  ...
  function initTargetManager() {
    ...
    class TargetManager {
      ...
      getNextHighestLowRatioTarget(currentTarget) {
        if (!currentTarget) {
          const firstTarget = targets.first();
          if (firstTarget.getRatio() < -0.25) {
            return firstTarget;
          }
          return null;
        }
        ...
        if (!nextTarget || nextTarget.getRatio() > -0.25) {
