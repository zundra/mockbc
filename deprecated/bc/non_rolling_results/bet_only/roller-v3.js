...
$(document).ready(function () {
    ...
    let html = `
        <div id="control-panel">
            ...
            <div class="control-group">
                <label>Wager</label>
                <input type="number" id="wager" value="0.0001" />
            </div>
    ...
    function defaultSession() {
        return {
            ...
            start: function(payout, stopOnMaxRolls, maxRolls, stopOnHitCount, hitCountTarget, stopOnProfit, profitTarget, stopOnMiss) {
                this.reset();
                this.state.stopped = false;
                this.payout = payout;
                this.stopOnMaxRolls = stopOnMaxRolls;
    ...
    function getStopOnMaxRolls() {
        return $("#stop-on-max-rolls").prop("checked");
    }
    ...
    function startBetting() {
        ...
        const stopOnMaxRolls = getStopOnMaxRolls();
        ...
        SESSION.start(payout, stopOnMaxRolls, maxRolls, stopOnHitCount, hitCountTarget, stopOnProfit, profitTarget, stopOnMiss);
        doBet();
    }
    ...
    function initRollHandler() {
        ...
        class RollHandler {
            ...
            detectLocalMinima(target) {
                if (!target || !target.leftTarget || !target.rightTarget) return null; // Skip edge cases

                const currentLSRatio = target.getLSRatio();
                const leftLSRatio = target.leftTarget.getLSRatio();
                const rightLSRatio = target.rightTarget.getLSRatio();

                if (currentLSRatio < leftLSRatio && currentLSRatio < rightLSRatio) {
                    console.log(`🔹 Local Min Detected: ${target.getPayout()}x with LSRatio ${currentLSRatio}`);

                    // Check for build-up (ensuring left and right targets form a valley)
                    const leftBuild = target.leftTarget.getLSRatio() >= currentLSRatio;
                    const rightBuild = target.rightTarget.getLSRatio() >= currentLSRatio;

                    if (leftBuild && rightBuild) {
                        console.log(`✅ Build-up Confirmed at ${target.getPayout()}x`);

                        // Recursively check if there's a pop
                        setTimeout(() => {
                            if (target.getLSRatio() > currentLSRatio * 1.5) { // Pop condition
                                console.log(`🚀 Pop Event! ${target.getPayout()}x target`);
                            }
                        }, 1000); // Delay to allow next rolls to process
                    }

                    return target;
                }

                // Recurse through all targets
                return this.detectLocalMinima(target.rightTarget);
            }

            findAndProcessMinima() {
                this.targets.forEach(target => this.detectLocalMinima(target));
            }
        }
