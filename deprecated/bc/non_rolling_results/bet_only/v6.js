...
;(function () {
    ...
    $(document).ready(function () {
        ...
        <div id="control-panel">
            ...
            <div id="watch-mode-control-wrapper">
                <div class="control-group">
    ...
    function evalHalts(result) {
        ...
        const crpTarget = ROLL_HANDLER.getCRPTarget();

        if (crpTarget) {
            const message = `Target ${crpTarget.getPayout()} hit the CRP zone at ${crpTarget.getRollCount()} rolls / ${crpTarget.getHitCount()} hits with ${crpTarget.getWindowSize() - crpTarget.getRollCount()} remaining in the window.`;
            console.log(message);
        ...
        if (ROLL_HANDLER.getTotalStrength() <= -200) {
            ROLL_HANDLER.targets.forEach((target) => {
                console.log(
                    `Target: ${target.getPayout()}, LS: ${target.getLosingStreak()}`,
                )
            })
            SESSION.stop(`Total strength <= -200`)
        }

        if (ROLL_HANDLER.supressedTargetsExceedThreshold(ROLL_HANDLER.getTargets(), 0, getStrengthPercentLimit())) {
            ROLL_HANDLER.targets.forEach((target) => {
                console.log(
                    `Target: ${target.getPayout()}, LS: ${target.getLosingStreak()}`,
                )
            })
            SESSION.stop(`Supressed targets exceeds threshold ${getStrengthPercentLimit()}`)
        }

        if (ROLL_HANDLER.supressedTargetsExceedThreshold(ROLL_HANDLER.getLowTargets(), 3, 1)) {
            ROLL_HANDLER.getLowTargets().forEach((target) => {
                console.log(
                    `Target: ${target.getPayout()}, LS: ${target.getLosingStreak()}`,
                )
            })
            SESSION.stop(`Supressed low targets exceeds threshold 1`)
        }
    ...
    function initWindowEvents() {
        $("#set-risk-on-btn").click(function () {
            if (stopTarget) {
                setPayout(stopTarget.getPayout())
                stopTarget = null
            }
    ...
    function initRollHandler() {
        ...
        class RollHandler {
            ...
            getCRPTarget() {
                return this.targets
                    .filter(target => target.hitRateExceedsExpectedRate(3) && target.getStrength() < -) // 3x CRP threshold
