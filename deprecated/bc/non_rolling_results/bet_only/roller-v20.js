...
// tuning
const HOUSE_EDGE = 0.04;     // bc.game ~4% now
const WINDOW_SIZE = 500;     // rolling window length
const Z = 2.05;              // ~98% CI; use 1.96 for 95%
const BUFFER = 0.003;        // 0.3% cushion
const MIN_TRIALS = 150;      // don't react too early
  ...
  function evalResult(result) {
    ...
    const inRecoveryMode = getInRecoveryMode();
  ...
  function setStreaks(result) {
    ...
    WATCH_PAYOUTS.forEach((entry) => {
      ...
      if (result >= entry.payout) {
        if (entry.streak < 0) {
          entry.pstreak = entry.streak;
        }
        entry.streak = Math.max(entry.streak + 1, 1);
      } else {
        if (entry.streak > 0) {
          entry.pstreak = entry.streak;
        }
        entry.streak = Math.min(entry.streak - 1, -1);
      }

      entry.streakDiff = entry.streak + payout;
      entry.ratio = entry.streakDiff / payout;
    ...
    if (!getRecoveryMode()) return;

    RECOVERY_PAYOUTS.forEach((entry) => {
      const payout = entry.payout;
      if (result >= payout) {
        entry.streak = 0;
      } else {
  ...
  function getRecoveryFlips() {
    ...
    const negativeTargets = WATCH_PAYOUTS
      .filter((t) => t.ratio < 0)
  ...
  function checkConditions(result) {
    ...
    if (inRecoveryMode && recoveryDelta > 0) {
      exitRecovery("Recovery successful, exiting recovery mode:", recoveryDelta);
      return;
    }
    ...
    WATCH_PAYOUTS.filter((t) => t.payout >= 5).forEach((entry) => {
      const contextString = getPostStreakHalt() ? "previous" : "current";
      const streak = getPostStreakHalt() ? entry.pstreak : entry.streak;
      const payout = entry.payout;
      const threshold = payout * getWatchLosingStreakMultiplier();
      const amplifiedThreshold = threshold * 2;
      ...
      if (entry.streak >= amplifiedThreshold) {
        halt(
          `Target ${payout} hit amplified losing streak threshold ${streak}`
        );
        return;
      }
    });
  ...
  function updateUI() {
    ...
    $("#recovery-profit-loss")
      ...
      .css({
        backgroundColor: `${
          recoveryDelta > 0 ? "green" : recoveryDelta < 0 ? "red" : "transparent"
        }`
      });
  ...
  function initWindowEvents() {
    ...
    $("#configure-high-roll-btn").click(function () {
      $("#risk-on").prop("checked", false);
      $("#hit-count-target").val(1);
      $("#watch-ls-multiplier").val(7.5);
      $("#ls-multiplier").val(7.5);
      $("#recovery-mode").prop("checked", true);
...
function populateWatchPayouts() {
  const a1 = Array.from({ length: 10 }, (_, k) => parseFloat((1.1 + k * 0.1).toFixed(2)));
  const a2 = Array.from({ length: 99 }, (_, k) => 2 + k);
  const a3 = [200,300,400,500,1000,2000,3000,4000,5000,10000,25000,50000,100000,500000,1000000];
  const p = [...a1, ...a2, ...a3];

  WATCH_PAYOUTS = p.map((pay) => {
    // theoretical success rate for payout pay
    const wStar = (1 - HOUSE_EDGE) / pay;

    return {
      payout: pay,
      // your existing fields (kept intact)
      streak: 0,
      streakBreached: false,
      pstreak: 0,
      ratio: 0,
      streakDiff: 0,

      // win-rate guard state
      wStar,
      ...
      // circular buffer for wins (true/false), plus counters
      wins: 0,
      n: 0,
      _buf: new Array(WINDOW_SIZE).fill(false),
      _head: 0,
      // result of guard check
      winRateBreached: false,
      // optional telemetry
      lowerBound: null,
      phat: 0
    };
...
/**
 * Call this on every crash result.
 * @param {number} outcome - the rolled multiplier (e.g. 3.42 means 3.42x)
 */
function updateWatchPayoutsOnRoll(outcome) {
  const threshold = getLosingStreakMultiplier();

  for (let i = 0; i < WATCH_PAYOUTS.length; i++) {
    const t = WATCH_PAYOUTS[i];

    // "win" if the crash reaches at least the payout
    const win = outcome >= t.payout;

    // circular window update
    if (t.n < t.windowSize) {
      // still filling
      t._buf[t._head] = win;
      if (win) t.wins += 1;
      t._head = (t._head + 1) % t.windowSize;
      t.n += 1;
    } else {
      // full: evict oldest, insert newest
      const evicted = t._buf[t._head];
      if (evicted) t.wins -= 1;
      t._buf[t._head] = win;
      if (win) t.wins += 1;
      t._head = (t._head + 1) % t.windowSize;
    ...
    // optional: keep your streaks in parallel
    t.streak = win ? 0 : t.streak + 1;

    // win-rate guard
    if (t.n >= t.minTrials) {
      t.phat = t.wins / t.n;
      const lower = wilsonLowerBound(t.wins, t.n, t.z);
      t.lowerBound = lower;
      t.winRateBreached = lower < (t.wStar - t.buffer);
    } else {
      t.phat = t.n ? t.wins / t.n : 0;
      t.lowerBound = null;
      t.winRateBreached = false;
    }

    // if you still want a streak-based kill as a backstop, keep it:
    t.streakBreached = t.streak >= (t.payout * threshold);

    // your NEW halting signal per payout:
    // t.winRateBreached === true
  }
}

/**
 * Example: global halting decision using the new guard.
 * You can OR or AND these depending on how strict you want it.
 */
function shouldHaltAll() {
  // halt if ANY watched payout shows statistically-degraded regime
  return WATCH_PAYOUTS.some(t => t.winRateBreached);
  // Or be stricter:
  // return WATCH_PAYOUTS.filter(t => t.winRateBreached).length >= 3;
}

  function populateRecoveryPayouts() {
    RECOVERY_PAYOUTS = Array.from(
      {
        length: 53
      },
      (v, k) => 1.5 + k * 0.01
    ).map((p) => ({ payout: p, streak: 0 }));
  }

  // function configureRiskOffMode() {
  //   $("#lows-ls-multiplier").val(6);
  //   setWager(0);
  //   inRecoveryMode = true;
  //   recoveryPL = 0;
  //   profitAtRecoveryStart = profit;
...
function getDynamicMaxLoss() {
  ...
  let controlPanel = `<div id="control-panel" style="
   ...
   <div class="control-group">
      ...
      <input id="recovery-mode" type="checkbox" checked/>
   </div>
   <div class="control-group">
