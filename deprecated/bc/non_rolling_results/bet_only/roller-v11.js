...
;(function () {
    "use strict"
    let beatOffset = 0 // ⏩ incremented on each roll
    ...
    const tempoHistoryMap = {} // 🔁 Track beats per target
    const maxBeats = 100
    ...
    $(document).ready(function () {
        ...
        <div id="control-panel">
            ...
            <div class="control-group">
                <label>Cycle On/Off</label>
                <input id="cycle-on-off" type="checkbox" />
            </div>

            <div class="control-group">
                <label>Cycle Off Mod</label>
                <input type="number" id="cycle-mod" value="100" />
            </div>
            <div class="control-group">
                <label>Cycle Off Rolls</label>
                <input type="number" id="cycle-off-rolls" value="10" />
            </div>
                <div class="control-group">
         ...
         <div class="control-group">
            ...
            <div class="control-group">
              ...
              <select id='roll-mode'>
                ...
                <option value="strength-cycle">Strength Cycle</option>
    ...
    function evalResult(result) {
        ...
        if (getCycleOnOff()) {
            if (SESSION.cycleOff) {
                if (SESSION.cycleOffRollCount !== getCycleOffRolls()) {
                    SESSION.cycleOffRollCount++;
                }
                else {
                    SESSION.cycleOff = false;
                    SESSION.cycleOffRollCount = 0;
                    setWager(SESSION.cycleOffWager);
                }
            } else if (SESSION.globalRollCount % getCycleMod() === 0) {
                SESSION.cycleOff = true;
                SESSION.cycleOffWager = getWager();
                setWager(0);
            }
        }
        updateUI()
        evalHalts()
        updateCanvas(result);
        if (window.__scrollTempoFrame) window.__scrollTempoFrame()
    }

function updateCanvas(result) {
  globalBeatOffset++; // Move one step per new roll

  ROLL_HANDLER.getTargets().forEach((target) => {
    const payout = target.getPayout();
    const hit = result >= payout;

    tempoHistoryMap[payout].push(hit);
    if (tempoHistoryMap[payout].length > 500) {
      tempoHistoryMap[payout].shift();
    }
  });
}

function drawTempoBeats(ctx) {
    const beatSpacing = 8
    let scrollOffset = 0
    ...
    function render() {
        ...
        const laneHeight = 20
        const maxBeats = Math.floor(ctx.canvas.width / beatSpacing)

        targets.forEach((target, index) => {
            const y = index * (laneHeight + 10) + 10
            const teo = 1 / (target * 1.05)
            const expectedInterval = Math.round(1 / teo)

            const beats = tempoHistoryMap[target]
            const recentBeats = beats.slice(-maxBeats)

            // Target Label
            ctx.fillStyle = "#ccc"
            ctx.font = "15px monospace"
            ctx.textAlign = "right"
            ctx.fillText(`${target}x`, 35, y + laneHeight / 2 + 4)

            // Ghost Beats (expected)
            ctx.fillStyle = "rgba(255, 255, 100, 0.2)"
            for (let i = 0; i < maxBeats; i++) {
                if ((i + scrollOffset) % expectedInterval === 0) {
                    const x = i * beatSpacing + 40
                    ctx.beginPath()
                    ctx.arc(x, y + laneHeight / 2, 3, 0, 2 * Math.PI)
                    ctx.fill()
                }
            }
            ...
            recentBeats.forEach((hit, i) => {
                if (hit) {
                    const x = i * beatSpacing + 40
                    ctx.beginPath()
                    ctx.arc(x, y + laneHeight / 2, 4, 0, 2 * Math.PI)
                    ctx.fill()
                }
            })
            ...
            ctx.lineTo(ctx.canvas.width, y + laneHeight / 2)
            ctx.stroke()
        })
    }

    // Call this once per new roll to keep timing perfect
    window.__scrollTempoFrame = () => {
        scrollOffset = (scrollOffset + 1) % 10000 // Prevent infinite growth
        render()
    }
    ...
    function evalHalts(result) {
      ...
      console.log(message)
        ...
        if (SESSION.globalRollCount < 100) return;


  if (
            Stats10.getVariance() <  0.5 && 
            Stats100.getVariance() <  10 && 
            Stats1000.getVariance() <  100 &&

            Stats10.getMean() < Stats100.getMean() - 1.5 && 
            Stats10.getMedian() < Stats100.getMedian() && 

            Stats10.getStandardDeviation() < 0.75 && 
            Stats100.getStandardDeviation() < 50 &&
            Stats1000.getStandardDeviation() < 100 &&

            Stats100.getMedian() < 1.9 &&
            Stats1000.getMedian() < 1.9) {
...
return;
        ROLL_HANDLER.targets.forEach((target) => {
            if (target.startsWeakZone()) {
                ...
                console.log(`Total Bubble Strength: ${bubbleStrength}`)

                if (leftEdge.getPayout() <= target.getPayout() && bubbleStrength < ROLL_HANDLER.weaknessThreshold) {
...
function initTempoCanvas() {
  // Create wrapper div (scrollable container)
  const wrapper = document.createElement("div")
  wrapper.id = "tempoWrapper"
  wrapper.style.position = "fixed"
  wrapper.style.bottom = "0"
  wrapper.style.left = "0"
  wrapper.style.width = "100%"
  wrapper.style.height = "00px"
  wrapper.style.overflowY = "auto"
  wrapper.style.zIndex = "9998"
  wrapper.style.background = "#111"

  // Create the canvas inside it
  const canvas = document.createElement("canvas")
  canvas.id = "tempoCanvas"
  canvas.width = window.innerWidth
  canvas.height = 1000 // 👈 make it tall enough to fit all rows
  ...
  // Optional: Update canvas width on resize
  window.addEventListener("resize", () => {
    canvas.width = window.innerWidth
  })

  drawTempoBeats(ctx) // Start rendering
}
    ...
    function isStrengthCycle() {
        return getRollMode() === "strength-cycle";
    }
    ...
    function getCycleOnOff() {
        return $("#cycle-on-off").prop("checked")
    }

    function getCycleMod() {
        return Number($("#cycle-mod").val())
    }

    function getCycleOffRolls() {
         return Number($("#cycle-off-rolls").val())
    }
    ...
    function initTarget() {
        ...
        class RiskOnTarget {
            constructor(payout, wager, baseWager, bankRollStartBalance) {
                this.bankRoll = new BankRoll(bankRollStartBalance)
                this.wager = Math.max(wager, baseWager)
                this.baseWager = Math.min(wager, baseWager)
            ...
            adjustWager() {
                if (this.rollCount < this.payout) return;

                if (this.getStrength() > 1 && this.wager > 0) {
                    this.holdWager = this.wager;
                    this.wager = 0;
                } else if (this.getStrength() < -1) {
                    if (this.wager === 0) {
                        if (this.getPL() < 0) {
                            this.wager = this.holdWager * 2;                        
                        } else {
            ...
            setWager() {
                ...
                const lsRatio = this.getLosingStreakAbs() / this.getPayout()
                this.wager = Math.max(
                    this.baseWager,
                    Math.min(MAX_WAGER, this.wager * (1.5 * lsRatio)),
                )
            }
            getStreak() {
                return this.streak
    ...
    function initRollHandler() {
        ...
        class Target {
            ...
            findBubbleEdges() {
                ...
                while (leftEdge.getLeftTarget() && leftEdge.getLeftTarget().getStrength() < 0) {
                    leftEdge = leftEdge.getLeftTarget();
                }

                // ✅ Traverse right while weakness continues
                while (rightEdge.getRightTarget() && rightEdge.getRightTarget().getStrength() < 0) {
            ...
            isSibling(otherTarget) {
                return this.getLeftTarget() === otherTarget || this.getRightTarget() === otherTarget;
            }
...
calculateWeaknessThreshold(strengths) {
    ...
    const meanStrength = strengths.reduce((sum, s) => sum + s, 0) / strengths.length;
    const variance = strengths.reduce((sum, s) => sum + Math.pow(s - meanStrength, 2), 0) / strengths.length;
    ...
    printReport() {
        const minStrengthObserved = Math.min(...this.strengthHistory);
        const meanStrength = this.strengthHistory.reduce((sum, s) => sum + s, 0) / this.strengthHistory.length;
        const variance = this.strengthHistory.reduce((sum, s) => sum + Math.pow(s - meanStrength, 2), 0) / this.strengthHistory.length;
        const stdDev = Math.sqrt(variance);

        console.clear();
        console.log("🚀 **ROLL HANDLER REPORT** 🚀");
        console.table([
            { Metric: "Total Rolls", Value: SESSION.getGlobalRollCount() },
            { Metric: "Min Strength Observed", Value: minStrengthObserved.toFixed(4) },
            { Metric: "Mean Bubble Strength", Value: meanStrength.toFixed(4) },
            { Metric: "Standard Deviation", Value: stdDev.toFixed(4) },
            { Metric: "Number of Halts Triggered", Value: this.haltCount }
        ]);
    }
