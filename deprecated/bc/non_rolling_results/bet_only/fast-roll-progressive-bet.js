/* Start Script */
// ==UserScript==
// @name         bc.game fast roll (progressive bet)
   ...
   initWindowEvents()
    ...
    function evalResult(result) {
        ...
        if (result >= getPayout()) {
            ...
            cyclePL += (wager * getPayout()) - wager;
            
            if (cyclePL >= 0) {
               cyclePL = 0;
                wager = baseWager;                
            } else {
                wager = Math.max(wager / 2, baseWager)
            }
            setWager(wager)
        } else {
            cyclePL -= wager;
        ...
        if (losingStreak !== 0 && losingStreak % rollMod === 0) {
            wager = getWager();
            wager *= 2;
            setWager(wager);
        }
