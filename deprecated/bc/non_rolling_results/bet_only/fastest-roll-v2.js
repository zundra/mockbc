...
function setStreaks(result) {
  const e = 0.02; // house edge as a fraction (2%)

  for (let i = 0; i < TARGETS.length; i++) {
    const t = TARGETS[i];

    // zero-mean per-roll score relative to the house edge
    const winBump  = (t.payout - 1) + e; // profit - EV, EV = -e
    const loseBump = -1 + e;             // profit - EV, EV = -e

    if (result >= t.payout) {
      // Hit: reset losing streak, carry/boost ratio by balanced win amount
      t.haltMessage = null;
      t.ls = 0;
      t.rawRatio = (t.rawRatio ?? 0) + winBump;
      t.ratio = t.rawRatio / t.payout;
    } else {
      // Miss: increment losing streak, apply balanced loss amount
      t.ls = (t.ls ?? 0) + 1;
      t.rawRatio = (t.rawRatio ?? 0) + loseBump;
      t.ratio = t.rawRatio / t.payout;
  ...
  function generateTargets() {
    return $("#watch-payouts")
      .val()
      .split(" ")
      .map((p) => ({ payout: parseInt(p), ls: 0, haltMessage: null, ratio: 0, rawRatio: parseInt(p), lsdiff: 0 }));
  }
