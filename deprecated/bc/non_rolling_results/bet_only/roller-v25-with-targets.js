/* Start Script */
// ==UserScript==
// @name         bc.game roller v25 (with targets)
...
(function () {
  ...
  function evalResult(result) {
    ...
    checkConditions(result);
      ...
      recoveryStateMachine.updateBalance(profit);
    
    const rsm = recoveryStateMachine.decideNextBet(2)

    if (rsm) {
    setWager(rsm.wager);
    setPayout(rsm.target.getPayout())
      console.log(`Payout: ${rsm.target.getPayout()}, State: ${rsm.state}, Wager: ${rsm.wager}`);
    }
  ...
  function processResult(result) {
    ...
    WATCH_TARGETS.forEach((target) => {
      ...
      // if (target.getIsCompressed())
      //   console.log(
      //     `Target = ${target.getPayout()}, isCompressed: ${target.getIsCompressed()}, "CompressionZone: ${
      //       target.compressionZone
      //     }`
      //   );
    });
  ...
  function checkConditions(result) {
    ...
    WATCH_TARGETS.filter((t) => t.payout >= 5).forEach((entry) => {
      ...
      if (entry.getIsExtremeCompressed() && Math.abs(streak) >= payout * 7.5) {
        halt(
  ...
  function initWindowEvents() {
    ...
    populateTargets();
    recoveryStateMachine = generateRecoveryStateMachine();
  }
  ...
  function generateRecoveryStateMachine() {
    class RecoveryStateMachine {
      constructor(baseBet = 1, targets = []) {
        this.state = "Normal";
        this.baseBet = baseBet;
        this.peakBalance = 0;
        this.currentBalance = 0;
        this.targets = targets; // all known targets live here
      }

      // --- State Transitions ---
      updateState(drawdown) {
        if (this.currentBalance >= this.peakBalance) {
          ...
          this.state = "ShallowDrawdown";
        } else {
          this.state = "DeepDrawdown";
        }
      }

      // --- Helper Methods ---
      isCompressionTrap(target) {
        return target.isCompressed && target.compressionZone != null;
      }

        findSafeAlly(target) {
          if (!target.compressionZone) return target; // no zone info, fallback to self

          const zoneMates = this.targets.filter(t => t.compressionZone === target.compressionZone);
          if (!zoneMates.length) return target; // nothing found, fallback to self

          return zoneMates.reduce((best, t) => {
            return Math.abs(t.getRatio()) > Math.abs(best.getRatio()) ? t : best;
          }, zoneMates[0]) || target; // absolute last fallback
        }


      lowBandTarget() {
        const result = (
          this.targets.find((t) => t.payout <= 2.0 && !t.isCompressed) ||
          this.targets.find((t) => t.payout <= 2.0)
        );
        return result;
      }

      // --- Decision Logic ---
      decideNextBet(payout) {
          const target = WATCH_TARGETS.find((t) => t.payout === payout);
        const ratio = target.getRatio();
        const drawdown = this.peakBalance - this.currentBalance;
        this.updateState(drawdown);

        switch (this.state) {
          case "Normal":
            return { target, wager: this.baseBet, state: this.state };

          case "ShallowDrawdown":
            if (ratio > -0.2) {
              return { target, wager: this.baseBet * 0.5, state: this.state  };
            }
            return { target, wager: this.baseBet, state: this.state  };

          case "DeepDrawdown":
            if (this.isCompressionTrap(target)) {
              const ally = this.findSafeAlly(target);
              return { target: ally, wager: this.baseBet, state: this.state  };
            }
            const low = this.lowBandTarget();
            return { target: low, wager: this.baseBet * 1.5, state: this.state  };
    ...
    return new RecoveryStateMachine(1, WATCH_TARGETS);
  }
