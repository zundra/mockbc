/* Start Script */
// ==UserScript==
// @name         bc.game roller v25.1 (with targets)
...
function updatePivotUI() {
  ...
  function updatePressureUI(pivot) {
    const el = document.getElementById("pressureFill");
    if (!pivot) {
      el.style.width = "0%";
      return;
    }

    const pressure = Math.abs(pivot.getPreviousTargetsRatioSum()); // bigger = more pressure
    const capped = Math.min(pressure, 100); // cap to avoid overflow
    const pct = (capped / 100) * 100; // normalize 0–100
  ...
  function getPivotTarget() {
    ...
    return pivot;
      ...
      // function getPivotTargetStats(pivotTarget) {
      //   let span = 0;
      //   let sum = 0;
...
function getPivotTargetStats(pivotTarget) {
  if (!pivotTarget) return { span: 0, sum: 0, avg: 0 };
  ...
  while (current && current.getRatio() < 0) {
    ...
    const prev = current.getPreviousTarget();

    // break if gap exists (non-consecutive payouts)
    if (prev && Math.abs(prev.getPayout() - current.getPayout()) > getResolution()) {
      break;
    }
...
// helper: figure out resolution based on config
function getResolution() {
  // assumes payouts array is sorted
  const diffs = [];
  const payouts = generateWatchPayouts
  for (let i = 1; i < this.payouts.length; i++) {
    diffs.push(this.payouts[i] - this.payouts[i - 1]);
  }
  return Math.min(...diffs); // smallest observed step
}
  ...
  function populateTargets() {
    class Target {
      constructor(
        payout,
        lookbacks = [10, 20, 30, 40, 50, 60, 70, 80, 90, 200]
      ) {
        this.payout = payout;
