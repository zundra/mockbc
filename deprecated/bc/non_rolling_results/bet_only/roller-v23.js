...
(function () {
  ...
  let wagerManager = initWagerManager();
  ...
  let controlPanel = `<div id="control-panel" style="
   ...
   <div class="control-group">
      <label>Use Wager Manager</label>
      <input id="use-wager-manager" type="checkbox"/>
   </div>
   <div class="control-group">
  ...
  function trySetWager(result) {
    if (!getUseWagerManager()) return;

    if (!wagerManager.isConfigured()) {
      wagerManager.configure(getPayout(), 1, getMaxWager())
    }
    
    wagerManager.push(result);
    console.log("WOULD SET WAGER", wagerManager.getWager())
  }
  ...
  function getRecoveryFlips() {
    ...
    const negativeTargets = WATCH_PAYOUTS.filter((t) => t.ratio < 0).sort(
  ...
  function getUseWagerManager() {
    return $("#use-wager-manager").prop("checked");
  }
  ...
  function initWagerManager() {
    class WagerManager {
      ...
      configure(payout, wager, maxWager) {
        this.reset();
        ...
        this.multiplier = 1 + (2 - this.payout);
      }
      ...
      getPL = () => this.profit;
      getTeo() {
        return 1 / (this.payout * 1.04);
      }

      setNextWager() {
        if (!this.isHit) {
          this.wager = Math.min(this.maxWager, this.wager * this.multiplier);
