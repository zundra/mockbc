...
(function () {
  ...
  let highSinceEntry = {result: 0, round: 0, 
  reset: function() {
    highSinceEntry.result = 0;
    highSinceEntry.round = 0;
  },
  isStalled: function(currentRound) {
    return currentRound - highSinceEntry.round > highSinceEntry.result;
  }};
  ...
  function evalRiskOn(result) {
    ...
    let nextPayout = payout;
    let wager = getWager();
    let maxWager = getMaxWager();
    let baseWager = getBaseWager();
    ...
    if (result >= payout) {
      ...
      // if (payoutPinned && losingStreak % Math.ceil(nextPayout / 2) === 0) {
      //   wager *= 2;
      // } else if (highSinceEntry.isStalled(rollCount)) {
      //   let nextWager = (baseWager * losingStreak * 1.01) / (nextPayout * 0.99);
      //   wager = Math.min(maxWager, nextWager);
      // } else {
      ...
      if (payoutPinned && losingStreak % Math.ceil(nextPayout / 2) === 0) {
        ...
        // This section replaces the need for isStalled entirely
        const payoutDelta = highSinceEntry.result - nextPayout;
        const rollsSinceHigh = Math.max(1, rollCount - highSinceEntry.round);
        const ascentSlope = payoutDelta / rollsSinceHigh;

        const baseClimbChance = 0.5;
        const dynamicMultiplier = Math.min(2, Math.max(0, ascentSlope * 2)); // [0–2]
        const climbChance = baseClimbChance * dynamicMultiplier;

        if (Math.random() < climbChance) {
          nextPayout = Math.min(nextPayout + 1, getMaxPayout());
        }

        // const nextWager =  Math.max(baseWager, (baseWager * losingStreak * 1.01) / (nextPayout * 0.99));
        if (losingStreak % Math.ceil(nextPayout / 2) === 0) {
          wager *= 2;
        }
        wager = Math.min(maxWager, wager);
      }

      setPayout(nextPayout);
      setWager(Math.min(getMaxWager(), wager));
    }

    if (result > highSinceEntry.result) {
      highSinceEntry.result = result;
      highSinceEntry.round = rollCount;
    }
  ...
  function evalRiskOff(result) {
    ...
    if (baseLosingStreak >= (basePayout * 2) || maxLosingStreak >= (maxPayout * 3)) {
      riskOn = true;
      baseLosingStreak = 0;
      maxLosingStreak = 0;
      setWager(getBaseWager())
