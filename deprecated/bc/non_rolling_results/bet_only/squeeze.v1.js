...
(function() {
    ...
    ">
        ...
        <div>Losing Streak: <span id="losing-streak"></span></div>
           ...
           <div class="control-group">
            <label>Squeeze Payout</label>
            <input type="number" id="squeeze-payout" value="10000" />
            </div>

           <div class="control-group">
            <label>Squeeze Wager</label>
            <input type="number" id="squeeze-wager" value="0.0001" />
            </div>
            </div>
...
`;
    ...
    function checkSqueeze(result) {
        if (result >= getPayout()) {
            setSqueeze(1, result, true)
            setMessage("Target hit during squeeze", getPayout(), "setting to ", squeezePayout);
             return;
        } else if (result >= Math.floor(getPayout() / 2)) {
             setSqueeze(rollCount < getMaxRolls() ? 1.5 : 2, result)
             setMessage(`Lower suqeeze target hit, resetting to ${squeezePayout}`);
             return;
        } else if (rollCount >= getMaxRolls()) {
             setSqueeze(4, Math.floor( Math.floor(squeezePayout / 2)))
             setMessage(`Lower suqeeze target hit, resetting to ${squeezePayout}`);
             return;
        }
    }

    function setSqueeze(wagerMultiplier, payout, reset = false) { 
        if (reset) {
            squeezeWager = getSqueezeWager();
            squeezePayout = Math.max(payout, getSqueezePayout())
        } else {
            squeezeWager = squeezeWager * wagerMultiplier;
            squeezePayout = payout;
         }
        
        setWager(squeezeWager);
        setPayout(squeezePayout);
        rollCount = 0;
    }
    ...
    function getMaxRolls() {
        return Math.floor(squeezePayout);
    }
  ...
  function getStopOnHitCount() {
    ...
    function getSqueezePayout(result) {
        return Number($("#squeeze-payout").val())
    }

    function getSqueezeWager() {
        return  Number($("#squeeze-wager").val());
    }
  ...
  function getHitCountTarget() {
    ...
    function startBetting() {
            ...
            squeezePayout = Math.floor(getPayout() / 2);
