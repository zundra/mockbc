...
<div id="control-panel">
  ...
  function defaultSession(rollHandler) {
    return {
      ...
      midLookback: 0,
      longLookback: 0,
      shortWinRateThreshold: 0,
      midWinRateThreshold: 0,
...
//        this.checkScoreThreshold()
        this.checkWinRateLosingStreakThreshold();
      },
      setRollConfigVariables(rollConfig) {
        ...
        this.shortWinRateThreshold = 50;
        this.midWinRateThreshold = 70;
        this.longWinRateThreshold = 90;
      },
      ...
      checkScoreThreshold() {
        ...
        if (targets.length != 0) {
          ...
          const message = `🟡 The following targets exceeded score threholds:\n\n${names}\n\n🛑 Session halted for review.`
      ...
      checkWinRateLosingStreakThreshold() {
        const targets = rollHandler.getTargetsExceedingWinRateStreakThresholds(
          this.minPayout,
          this.shortWinRateThreshold,
          this.midWinRateThreshold,
          this.longWinRateThreshold
        )

        if (targets.length != 0) {
          const names = targets.map((t) => t.getPayout()).join(", ")
          const message = `🟡 The following targets exceeded losing streak win rate threholds:\n\n${names}\n\n🛑 Session halted for review.`
   ...
   return this.streakAvg;
      ...
      getBadStreakThreshold() {
        ...
        exceedsAllThresholds(
            ...
            return this.getRiskScore() < -scoreThreshold && 
            this.getStreak() < this.getBadStreakThreshold() &&
            (this.getStreakAvg() < 0 && Math.abs(this.getStreakAvg()) > (3 * this.getPayout()));
        }


        exceedsLosingStreakThresholds() {
         if (this.rollCount < this.payout * 10) return false

         return this.getStreak() < this.getBadStreakThreshold() &&
            (this.getStreakAvg() < 0 && Math.abs(this.getStreakAvg()) > (this.getPayout()))
        }


        exceedsWinRateThreholds(shortThreshold, midThreshold, longThreshold) {
         if (this.rollCount < this.payout * 10) return false
         return this.getWinRatePercentOfTeo(this.getShortLookback()) < shortThreshold &&
            this.getWinRatePercentOfTeo(this.getMidLookback()) < midThreshold &&
            this.getWinRatePercentOfTeo(this.getLongLookback()) < longThreshold;
        }

        exceedsLosingStreakWinRateThresholds(
            ...
            return this.exceedsWinRateThreholds(shortThreshold, midThreshold, longThreshold);
        }
    ...
    isWindUpTight() {
      ...
      getTargetsExceedingWinRateStreakThresholds(
        minPayout,
        shortThreshold,
        midThreshold,
        longThreshold
        ...
        return this.targets.filter(
          (target) =>
            target.getPayout() >= minPayout &&
            target.exceedsWinRateThreholds(shortThreshold, midThreshold, longThreshold) &&
            target.exceedsLosingStreakThresholds()
        )
      }
