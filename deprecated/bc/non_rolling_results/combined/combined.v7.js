...
const WATCH_PARTITIONS = createWatchPartitions();

function createWatchPartitions() {
    const labels = ["lows", "mids", "highs", "ultras", "megas", "gigas"];
    let end = 46;
    let size = 46;
    let start = 0;
    let partitions = {};
  ...
  let controlPanel = `<div id="control-panel" style="
   ...
   <div class="control-group">
      <label>View Partition</label>
        <select id="watch-partition">
          <option value="lows">Low (1x–10x)</option>
          <option value="mids">Mid (10x–100x)</option>
          <option value="highs">High (100x–1,000x)</option>
          <option value="ultras">Ultra (1k–10k)</option>
          <option value="megas">Mega (10k–100k)</option>
          <option value="gigas">Giga (100k–1M)</option>
        </select>
...
function getRollsPerSecond() {
  ...
  function initWindowEvents() {
        $("#watch-partition").change(function () {
          const partition = $(this).val();
          [start, end] = WATCH_PARTITIONS[partition];
        });
  ...
  function populateWatchPayouts() {
    const payouts = [1.5, 1.6, 1.7, 1.8, 1.9, 2, 3, 4, 5, 6, 7, 8, 9, 10, 15, 20,  25, 30, 40, 50, 60, 70, 80, 90, 100, 200, 300, 400, 500, 1000, 2000, 3000, 4000, 5000, 10000, 25000, 50000, 100000, 500000, 1000000];
    WATCH_PAYOUTS = payouts.map((p) => ({ payout: p, streak: 0}));
  }
  ...
  function populateWatchPayouts2() {
    const WATCH_PARTITIONS = {
  lows: [0, 90],         // 1.0–10.0
  mids: [91, 181],       // 10–100
  highs: [182, 272],     // 100–1000
  ultras: [273, 363],    // 1k–10k
  megas: [364, 454],     // 10k–100k
  gigas: [455, 545]      // 100k–1M
};
    ...
    const payouts = [...p0, ...p1, ...p2, ...p3, ...p4, ...p5];
    WATCH_PAYOUTS = payouts.map((p) => ({ payout: p, streak: 0, hitCount: 0, teo: calculateTeo(p)}));
  }
...
function renderWatchTargetTable(data) {
  ...
  data.forEach(entry => {
    ...
    const row = `
      <tr style='background-color: ${rowColor}'>
        ...
        <td><span id="losing-streak-${sanitized}">${streak}</span></td>
        <td><span id="losing-streak-${sanitized}-ratio">${ratio.toFixed(2)}</span></td>
        <td><span id="losing-streak-${sanitized}-diff">${diff.toFixed(2)}</span></td>
      </tr>
    `;
    $tbody.append(row);
  });
...
function doRenderTable() {
    const visibleRows = WATCH_PAYOUTS.slice(start, end + 1); // inclusive end
    renderWatchTargetTable(visibleRows);
}
