...
initWindowEvents();
 ...
 function getVelocityData(target) {
  ...
  const index = Math.min(gaugeBar.length - 1, Math.floor((prClamped / 2) * gaugeBar.length));
  ...
  if (isSlow && stats.getRollCount() > 30) {
    ...
    recoveryHtml = `
      <br><span style="color:${feasible ? "#ffcc00" : "#ff4444"}">
        ...
        ⏫ Need ${(requiredRate * 100).toFixed(2)}% hit rate (${(requiredRate / teo).toFixed(2)}× TEO)
      </span>`;
  }

  const html = `
    <div style="margin-bottom:8px;padding:4px;background:${bg};border-left:4px solid ${color};">
      <strong>${stats.getPayout()}x</strong> →
      R: ${stats.getRollCount()}, H: ${stats.getHitCount()}<br>
      Win Rate: ${(winRate * 100).toFixed(2)}% (TEO: ${(teo * 100).toFixed(2)}%)<br>
      Speed: ${(pr * 100).toFixed(1)}% of TEO [${gauge}] (${relativeSpeed})
      ${isStuck ? `<br>🚨 Long LS (${ls}) + Slow` : ""}
      ${recoveryHtml}
    </div>
  `;
  ...
  function generateTargets() {
    ...
    class Target {
      constructor(payout) {
        this.payout = payout;
        this.stats = this.payout <= 96 ? new Stats(100, payout) : new ScalarStats(payout)
      ...
      evaluateSnapshot() {
        ...
        const zScore = n > 0 ? (x - expectedHits) / Math.sqrt(n * p * (1 - p)) : 0;
      ...
      getHitCount = () => this.hitCount;
      getRollCount = () => this.rollCount;
      getHTMLID = () => this.payout.toString().replace(/\./g, "_").replace(/[^\w-]/g, "");
    }
