/* Start Script */
// ==UserScript==
// @name         bc.game (UI Find Low Recovery) v9
...
function getTextColor(streak, target) {
    ...
    function updateUI() {
        ...
        targets.forEach(entry => {
            ...
            const riskScore = entry.getRiskScore(winRateChangeShort, winRateChangeMed, winRateChangeLong);
            ...
            if (row.length === 0) {
                ...
                row.append(targetLabel, blockContainer, strengthContainer, winRateContainerShort, winRateContainerMed, winRateContainerLong, riskScoreContainer);
                table.append(row);
            }
            ...
            riskScoreContainer.text(`${riskScore.toFixed(2)}`)
            .css({backgroundColor: getRiskScoreColor(riskScore)});
...
function createWinRateBar(winRateContainer, winRateChange) {
    ...
    function initRollHandler() {
        const payouts = [1.01, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.6, 1.7, 1.8, 1.9, 2, 3];
        class HistoricalHit {
