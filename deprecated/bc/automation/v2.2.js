  ...
  function generateTargetManager() {
    class Target {
      ...
      addResult(result) {
        ...
        if (this.results.length === Math.ceil(this.payout * 100)) this.results.shift();
 ...
 getDynamicWager(currentWager) {
  ...
  return Math.max(0.0001, wager); // enforce safety floor
      ...
      isRiskOnReady(baseLsThreshold, fastThreshold, slowThreshold) {
        return this.getHitDelta(Math.ceil(this.payout * 5)) <= -5 && this.getHitDelta(Math.ceil(this.payout * 10)) < -2 && this.getHitDelta(Math.ceil(this.payout * 50)) < 0
      }
    }
