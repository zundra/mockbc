...
(function () {
  ...
  */
  ...
  function evalRiskOn(result) {
    ...
    const riskOnProfit = riskOnTarget.getPL();
//console.log("Profit", riskOnProfit, getMaxLoss(), 'profit < 0', riskOnProfit < 0, "Math.abs(riskOnProfit) >= getMaxLoss()", Math.abs(riskOnProfit) >= getMaxLoss(), "!recoveryTarget", !recoveryTarget)

    if (!recoveryTarget && riskOnProfit < 0 && Math.abs(riskOnProfit) >= getMaxLoss()) {
      console.log(`[Entering recovery] Profit: ${riskOnProfit}`)
      recoveryTarget = initRecoveryTarget(3, 0.1, Math.abs(riskOnProfit));
      setWager(recoveryTarget.getWager());
      setPayout(recoveryTarget.getPayout());
      return;
    }


    if (recoveryTarget) {
      if (recoveryTarget.getRecoveryComplete()) {
        recoveryTarget = null;
        if (riskOnTarget) {
      ...
      } else if (recoveryTarget.getRecoveryFail()) {
...
function initRecoveryTarget(payout, wager, recoveryAmount) {

    class RecoveryTarget {
      constructor(payout, baseWager, recoveryAmount) {
        this.payout = payout;
      ...
      push(result) {
        ...
        if (result >= this.payout) {
          ...
          if (this.profit >= this.recoveryAmount) {
            console.log(`Recovery Complete: ${this.profit}`);
         ...
         if (this.profit < 0 && Math.abs(this.profit) >= (this.recoveryAmount * 3)) {
            console.log(`Recovery FAIL: ${this.profit}`);
      ...
      defenseStep() {
        if (this.retriesLeft > 0) {
          ...
          if (this.losingStreak >= this.retryBudget * 4) {
            this.mode = "attack";
            this.escalationLevel = 0;
          }
        }
      ...
      getRecoveryComplete = () => this.recoveryComplete;
      getRecoveryFail = () => this.recoveryFail;
      getPayout = () => this.payout;
      getWager = () => this.wager;
    }

      return new RecoveryTarget(payout, wager, recoveryAmount)
}
