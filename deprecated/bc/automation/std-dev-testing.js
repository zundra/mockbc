...
(function () {
  ...
  const scenarios = {
    ...
    "Volatility Expansion": (target) => ({
      riskOnCheck: () => target.volatilityState.shouldRiskOn(),
      riskOffCheck: () => Math.abs(target.getRiskOnLosingStreak()) === 0
    }),
    // "Median(100) < 1.92": (target) => ({
    //   riskOnCheck: () => target.stats.getMedian(100) < 1.92,
    //   riskOffCheck: () =>
  ...
  */
  ...
  function createVolatitityState() {
    class RollingStats {
      constructor(size) {
        this.size = size;
    ...
    class VolatilityState {
      constructor() {
        ...
        this.releaseSignal = false;

        this.threshold = 0.5;
        this.releaseThreshold = 0.8;
        this.confirmRun = 3;
      }
      ...
      push(value) {
        ...
        if (this.stats.count < 100) return;

        const stdShort = this.stats.getStdDev(10);
        const stdMid = this.stats.getStdDev(100);
        const ratio = stdMid === 0 ? 1 : stdShort / stdMid;

        if (ratio < this.threshold) {
          this.run++;
          if (this.run >= this.confirmRun) {
            this.compressed = true;
          }
        } else {
          if (this.compressed && ratio >= this.releaseThreshold) {
            this.releaseSignal = true;
          }
          this.run = 0;
          this.compressed = false;
        }
  ...
  function initTargetManager(scenario) {
    ...
    class Target {
      constructor(payout, bankRoll, scenarioName, scenarioFn) {
        ...
        this.volatilityState = createVolatitityState();
      ...
      push(result) {
        ...
        this.volatilityState.push(result);
      ...
      getRollsTilRiskOff = () =>
        this.rollsTilRiskOff.length === 0 ? 0 : this.rollsTilRiskOff.last();
      getAvgRollsTilRiskOff = () => this.getMean(this.rollsTilRiskOff);
      getMaxRollsTilRiskOff = () =>
        this.rollsTilRiskOff.length === 0
          ? 0
          : Math.max(...this.rollsTilRiskOff);
      // From stats class
