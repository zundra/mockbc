  ...
  function initTargetManager(scenario) {
    ...
    class Target {
      constructor(payout, bankRoll, scenarioFn) {
        ...
        this.riskOn = false;
        this.maxWager = 0;
        this.upperPairedTarget = null;
        this.rollOverCount = 0;
        this.rollOverDivisor = 1;
      ...
      setUpperPairedTarget(t) {
        this.upperPairedTarget = t;
      }
      
      clearUpperPairedTarget() {
        this.upperPairedTarget = null;
      }
      ...
      push(result) {
        ...
        if (this.upperPairedTarget && result >= this.upperPairedTarget.getPayout()) {
          this.clearUpperPairedTarget();
        }
      ...
      shouldGoRiskOff() {
        return !this.upperPairedTarget || this.getPL() >= (this.payout * this.wager * 3)
      }
      ...
      evalRiskOn() {
        ...
        if (ls > 0 && ls % Math.ceil(this.payout === 0)) {
            this.rollOverCount++;
        }


        if (this.rollOverCount === 1) {
            this.rollOverDivisor = 2;
        } else if (this.rollOverCount === 2) {
          this.rollOverDivisor = 3;
        }

        if (ls % Math.ceil(this.payout / this.rollOverDivisor) === 0) {
//          this.wager = Math.min(this.baseWager * 100, this.wager * 2);
         // this.wager *= 2;
        }
    ...
    class TargetManager {
      constructor(targets, bankRoll) {
        ...
        this.lowerTargetRatio = null;
        this.lowerTargetPayout = null;
        this.belowRatioPair = {lower: null, upper: null}
      }
  ...
  `;
      ...
      setBelowRatioPair() {
        const upperTarget = this.targets.find((t) => t.getRatio() < -3);

        if (!upperTarget) return;

        const belowRatioTargets = this.targets.filter(
          (t) =>
            t.getRatio() < lowerTarget.getRatio() &&
            t.getPayout() >= lowerTarget.getPayout() + 5
        );

        if (belowRatioTargets.length === 0) return;

        lowerTarget.setUpperPairedTarget(belowRatioTargets[0]);
      }

      getNextRiskTarget() {
        return this.targets.find((target) => target.upperPairedTarget !== null);
      }
