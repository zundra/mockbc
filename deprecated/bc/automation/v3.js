  ...
  function evalResult(result) {
    ...
    if (result >= payout) {
      ...
      if (profit >= getProfitTarget()) {
        halt(`Profit target hit: Profit ${profit.toFixed(4)}, Peak ${peakProfit.toFixed(4)}`);
        return;
      }  
    } else {
      profit -= wager;
      ...
      if (profit <= peakProfit - getMaxLoss()) {
        halt(`Trailing max loss hit: Profit ${profit.toFixed(4)}, Peak ${peakProfit.toFixed(4)}`);
        return;
      }
    }
    ...
    if (!riskOnTarget) {
      for (let i = 0; i < targets.length; i++) {
        ...
        if (
          ...
          console.log(`[Going risk on with] Payout: ${target.getPayout()}`);
          riskOnTarget = target;
  ...
  function generateTargetManager() {
    class Target {
      ...
      addRiskOnResult(result) {
        if (result >= this.getPayout()) {
          ...
          this.profit += this.payout * this.wager - this.wager;
           if (this.profit > this.baseWager * this.payout) {
            console.log("Going risk off with winner on payout", this.getPayout());
            this.goRiskOff();
          } else {
            console.log("Hit but still negative, moving to baseWager", this.getPayout(), this.wager);
            this.wager = Math.max(this.wager / 2.5, this.baseWager);
          }
        } else {
        ...
        if (this.riskOnLosingStreak % Math.floor(this.getPayout() / 2) === 0) {
          this.attemptCount++;
          ...
          if (
            ...
            console.log(
              "Going risk off with loser on payout",
              this.getPayout()
            );
            this.goRiskOff();
            return;
      ...
      streakExceedsThreshold = (lsThreshold) => this.getLosingStreakAbs() >= this.getPayout() * lsThreshold;
      getTeo = () => this.teo;
      ...
      isRiskOnReady = (lsThreshold, fastThreshold, slowThreshold) =>
        this.rollCount >= 100 &&
        this.streakExceedsThreshold(lsThreshold) &&
...
<div class="control-group">
  ...
  <input type="number" id="risk-pct" value="5" />
      ...
      <input type="number" id="fast-win-rate-threshold" value="75"  />
   </div>

    <div class="control-group">
      <label>Slow Win Rate Threshold</label>
      <input type="number" id="slow-win-rate-threshold" value="90"  />
   </div>
