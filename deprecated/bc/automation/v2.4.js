...
(function () {
  ...
  function generateTargetManager() {
    class Target {
      ...
      addRiskOnResult(result) {
        ...
        if (result >= this.getPayout()) {
          ...
          if (this.wagerPinned && this.getPL() < 0) {
        ...
        const currentHitDelta = this.getHitDelta(Math.ceil(this.payout * 5));

        //   if (currentHitDelta < this.hitDeltaOnEntry) {
        //     this.hitDeltaOnEntry = currentHitDelta;
        ...
        if (this.riskOnLosingStreak % Math.ceil(this.payout / 2) === 0) {
          this.wager = this.getProgressiveWager();
        }

        if (this.riskOnRollCount % Math.ceil(this.payout) === 0) {
          this.attemptCount++;
        }
      ...
      isRiskOffReady() {
        if (this.wagerPinned && this.getPL() > 0) {
          console.log(
            `[Exit on pinned wager] Payout: ${this.payout} Wager: ${this.wager}`
          );
          return true;
        }
        if (
          this.getHitDelta(Math.ceil(this.payout * 5)) >
            this.hitDeltaOnEntry + 1 &&
          this.getPL() > 0
        ) {
          //     console.log(`[Hit delta moved to positive] Payout: ${this.payout}, Short:  ${this.getHitDelta(Math.ceil(this.payout * 5))}, Attempt Count: ${this.attemptCount}`);
          return true;
        }

        // if (this.getHitDelta(Math.ceil(this.payout * 5)) > 0 && this.getHitDelta(Math.ceil(this.payout * 10)) > 0) {
        //                console.log(`Hit delta moved to positive Short:  ${this.getHitDelta(Math.ceil(this.payout * 5))}, Long: ${this.getHitDelta(Math.ceil(this.payout * 10))}`);
        //        return true;
        //    }
        ...
        if (
          this.attemptCount === 2 &&
          this.getPL() > this.getPayout() * this.baseWager * 1.25
        ) {
      ...
      isRiskOnReady() {
        ...
        // }
        return (
          this.getHitDelta(Math.ceil(this.payout * 5)) <= -3 &&
          this.getHitDelta(Math.ceil(this.payout * 10)) < -2 &&
          this.getHitDelta(Math.ceil(this.payout * 50)) < 0
        );
      }
    }
    ...
    class TargetManager {
      ...
      getNextRiskOnTarget() {
        if (!this.volatilityCompressed()) return null;
      ...
      volatilityCompressed() {
        ...
        return stats10Mean < stats100Mean - 1 &&
        stats10variance < 0.5 &&
        stats10StdDev < 1 && stats10Mean < 1.92;
      }
    }
