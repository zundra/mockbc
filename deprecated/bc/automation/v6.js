...
(function () {
  ...
  TARGET_MANAGER.processRiskOnQueue();
  ...
  function evalResult(result) {
    ...
    if (TARGET_MANAGER.getPotentialTargets().length > 0) {
      console.log(TARGET_MANAGER.getPotentialTargets().length, "len");
    }
    ...
    if (!riskOnTarget) {
      ...
      if (riskOnTarget) {
        ...
        console.log("NEXT RISK ON TARGET", riskOnTarget.getPayout())
      }
    } else {
      console.log("isRiskOffReady", riskOnTarget.getRiskOnLosingStreak(), riskOnTarget.getRiskOn())
      if (riskOnTarget.isRiskOffReady() || !riskOnTarget.getRiskOn()) {
  ...
  function generateTargetManager() {
    class Target {
      ...
      enterPotentialRiskOn() {
        this.potentialRiskOn = true;
      }
      ...
      isPotentialRiskOn = () => this.potentialRiskOn;
      ...
      addRiskOnResult(result) {
        if (result >= this.getPayout()) {
          ...
          // Risk-off only if we've fully pulled back to baseline
          if (this.attemptCount === 0) {
            console.log(
              `[Pullback Fired V4 (${rollCount})] Reset to base → going risk off. ` +
                `Profit=${this.profit.toFixed(6)}, Drawdown=${drawdown.toFixed(
                  6
                )}, ` +
                `PotentialWin=${potentialWin.toFixed(
                  6
            ...
            console.log(
              `[Pullback Fired V4 (${rollCount})] Wager=${this.wager}, ` +
                `Drawdown=${drawdown.toFixed(
                  6
                )}, PotentialWin=${potentialWin.toFixed(6)}, ` +
                `Ratio=${coverageRatio.toFixed(2)}, Attempt=${
                  this.attemptCount
                }`
            );
          }
      ...
      isRiskOnReady(baseLsThreshold) {
        const dynamicThreshold = this.dynamicStreakThreshold(baseLsThreshold);
        return this.streakExceedsThreshold(dynamicThreshold);
    ...
    class TargetManager {
      ...
      getPotentialTargets = () => this.targets.filter((t) => !t.getRiskOn() && !t.isPotentialRiskOn() && t.isRiskOnReady());

      async queueRiskOnCandidates() {
        const targets = this.getPotentialTargets();

        if (targets.length === 0) return;
        await Promise.all(
          targets.flat().map((target) => target.enterPotentialRiskOn())
        );
      }

      async processRiskOnQueue() {
        while (true) {
          this.reapInvalidPotentialRiskOnTargets(); // Cleanup invalid targets
          await this.queueRiskOnCandidates();
      ...
      reapInvalidPotentialRiskOnTargets() {
        this.targets.forEach(target => {
            if (!target.getRiskOn() && target.isPotentialRiskOn() && !target.isRiskOnReady()) {
                target.exitPotentialRiskOn(); // Reset back to risk-off state
            }
        });
    }

          getNextRiskOnTarget() {
        return this.getPotentialTargets().first();
      }
...
function getStopOnMaxRolls() {
  ...
  if (!maxRolls || maxRolls === 0) return false;
  return true;
}
