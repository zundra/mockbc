...
(function () {
  ...
  const SCENARIO = parseInt($("#scenario").val());
  const MAXROLLS = parseInt($("#max-rolls").val());
  ...
  function evalResult(result) {
    ...
    if (!riskOnTarget) {
      riskOnTarget = TARGET_MANAGER.getNextRiskOnTarget();
      if (riskOnTarget) {
        riskOnCount++;
  ...
  function updateUI(result) {
    ...
    $("#risk-on-count").html(riskOnCount);
    $("#running-scenario").html(SCENARIO);
  ...
  function generateTargetManager() {
    class Target {
      ...
      setPivotTargetPayout(payout) {
        this.pivotTargetPayout = payout;
      }

      getPivotTargetPayout = () => this.pivotTargetPayout;
      ...
      getPreviousTargetsRatioSum() {
        if (!this.previousTarget) return 0;
        return this.sumPreviousRatios(this.previousTarget);
      }

      getPreviousTargetsNegativeStats() {
        let span = 0;
        let sum = 0;
        let t = this.previousTarget;

        while (t && t.getRatio() < 0) {
          sum += t.getRatio();
          span++;
          t = t.getPreviousTarget();
        }

        return {
          span, // how many previous targets were negative
          sum, // total sum of their ratios
          avg: span > 0 ? sum / span : 0
        };
      }

      sumPreviousRatios(target) {
        if (!target) return 0; // ✅ null/undefined guard
        return (
          target.getRatio() + this.sumPreviousRatios(target.getPreviousTarget())
        );
      }
      goRiskOff() {
        console.log(
          `Risk off: profit ${profit} on payout ${this.getPayout()}, max wager was ${
            this.maxWager
          }, Current Max Drawdown: ${maxDrawDown}`
        );
        this.wager = 0;
        this.riskOn = false;
        this.riskOnLosingStreak = 0;
      ...
      addResult(result) {
        ...
        // if (this.payout === 4) {
        //           console.log("getPreviousTargetsNegativeStats", this.getPreviousTargetsNegativeStats())
        // }
      ...
      addRiskOnResult(result) {
        if (result >= this.getPayout()) {
          ...
          maxDrawDown = Math.min(maxDrawDown, this.profit);
        }
      }
      ...
      isRiskOffReady() {
        ...
        const pivotTarget = TARGET_MANAGER.getPivotTarget(-3);
        return (this.getPL() >= 0 || !pivotTarget)

        // return (pivotTarget.getPayout() !== this.getPivotTargetPayout())

        // //if (TARGET_MANAGER.pivotSequenceChanged())
        // console.log("TARGET_MANAGER.getPivotChanged()", TARGET_MANAGER.getPivotChanged())
        // return TARGET_MANAGER.getPivotChanged()
        // if (TARGET_MANAGER.getPivotChanged()) return true;

        // // return (this.getRiskOnLosingStreak() >= this.payout);
      }
    }
    ...
    class TargetManager {
      constructor(payouts, decay = 0.95) {
        ...
        for (let i = 0; i < this.targets.length; i++) {
          ...
          const next = this.targets[i + 1] || null;
          const prev = this.targets[i - 1] || null;
      ...
      getPivotChanged = () => this.pivotChanged;

      getPivotTarget(minRatio, previousTarget = null) {
        let pivot = null;
      ...
      getPivotTargetStats() {
        const pivotTarget = this.getPivotTarget(-3);
      ...
      getNextRiskOnTarget() {
        if (SCENARIO === 1) {
          const pivot = this.getPivotTarget(-3);
          if (pivot) {
            const stats = this.getPivotTargetStats(pivot);

            console.log(
              `[Pivot Stats] payout=${pivot.getPayout()}, span=${
                stats.span
              }, sum=${stats.sum.toFixed(2)}, avg=${stats.avg.toFixed(2)}`
            );

            // Example entry filter: only take pivots with breadth + intensity
            if (stats.span >= 3 && stats.avg < -2.0) {
              return pivot;
            }
          }
        } else if (SCENARIO === 2) {
          const zone = this.getPivotPressureZone();
          if (zone && zone.zone.length !== 0) {
            return zone.zone.first();
          }
          return null;
        }
      }

      getPivotPressureZone() {
        const pivotTarget = this.getPivotTarget(-3);
        ...
        while (current && current.getRatio() < 0) {
          current.setPivotTargetPayout(pivotTarget.getPayout())
          zone.push(current);
          current = current.getPreviousTarget();
        }
        ...
        return {
          pivot: pivotTarget,
          zone: zone.reverse(), // lowest → pivot
          span: zone.length,
          sum: zone.reduce((a, t) => a + t.getRatio(), 0),
          avg:
            zone.length > 0
              ? zone.reduce((a, t) => a + t.getRatio(), 0) / zone.length
              : 0
        };
      }
    }
  ...
  function injectControlPanel() {
   ...
   <div id="stats" class="stats-box" style="
      ...
      text-align: center;
      ">
           <div>Senario: <span id="running-scenario"></span></div>
      ...
      <div>Risk On Count: <span id="risk-on-count"></span></div>
