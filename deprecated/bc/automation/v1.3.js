...
(function () {
  ...
  function evalResult(result) {
    ...
    if (!riskOnTarget) {
      riskOnTarget = TARGET_MANAGER.getPivotTarget();
      if (riskOnTarget) {
  ...
  function generateTargetManager() {
    class Target {
      ...
      addRiskOnResult(result) {
        if (result >= this.getPayout()) {
          ...
          if (this.getRiskOnLosingStreak() >= this.getPayout() * 1.5) {
            console.log(
              `Target ${this.payout} going risk off on losing streak ${this.riskOnLosingStreak}`
            );
            this.goRiskOff();
          }
        }
    ...
    class TargetManager {
      ...
      getPivotTarget() {
        ...
        if (pivot) console.log("pivot ratio", pivot.getRatio());
        return pivot;
      }
