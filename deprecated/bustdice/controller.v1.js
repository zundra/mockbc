...
<div id="control-panel">
  ...
  function updateUI() {
    ...
    targets.forEach((entry) => {
      ...
      const winRateShortZscore = entry.getWinRateZScore(entry.getShortLookback());
      ...
      if (row.length === 0) {
        ...
        const shortWinRateZscoreContainer = $("<td>")
         ...
         .css({ backgroundColor: winRateShortZscore < -2 ? "red" : "transparent"});
         ...
         const longWinRateZscoreContainer = $("<td>")
          ...
          .css({ backgroundColor: winRateLongZscore < -2 ? "red" : "transparent"});
  ...
  function initRollHandler() {
    ...
    class RollingWindow {
      ...
      isReliablyUnderperforming(lookback = this.count, zThreshold = -2) {
    ...
    class Target {
      ...
      getWinRateZScore(windowSize) {
        return this.rollingWindow.getZScore(windowSize);
      }

      isReliablyUnderperforming(windowSize, zThreshold = -2) {
        return this.rollingWindow.isReliablyUnderperforming(windowSize, zThreshold)
      }

      multipleWindowsReliablyUnderperforming() {
         ...
         this.isReliablyUnderperforming(this.getLongLookback());
      }
      ...
      exceedsAllThresholds(scoreThreshold) {
        ...
        const mid = this.getHitPoolDeviationScoreLookback(
          this.getMidLookback()
        );
      ...
      getTargetsUnderPerformingWinRateZScore(minPayout) {
        return this.targets.filter((target) => {
          return (
            target.isPastRollThreshold() &&
            target.getPayout() >= minPayout &&
            target.multipleWindowsReliablyUnderperforming()
          );
        });
      }
